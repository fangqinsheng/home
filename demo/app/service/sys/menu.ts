...
export default class SysMenuService extends BaseService {
    ...
    async addOrUpdate (param) {
        ...
        const users = await this.nativeQuery('select b.userId from sys_role_menu a left join sys_user_role b on a.roleId = b.roleId where a.menuId = ? group by b.userId', [ param.id ]);
        if (!_.isEmpty(users)) {
    ...
    async getMenus (roleIds) {
        ...
        SELECT
            a.id,a.parentId,a.name,a.type,a.icon,a.orderNum,a.router,a.viewPath,a.keepAlive
        FROM
            sys_menu a
